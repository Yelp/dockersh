...
func realMain() int {
	...
	if err != nil {
		_, err = dockerstart(config, config.ContainerName, config.ImageName, config.DockerSocket, config.MountTmp, config.MountDockerSocket, config.Entrypoint, config.Cmd, config.DockerOpt)
		if err != nil {
